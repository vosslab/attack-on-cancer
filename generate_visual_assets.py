...
def parse_sheet(path: pathlib.Path) -> VisualSheet:
	...
	if kind not in EXPECTED_CATALOG or KEY_PATTERN.fullmatch(key) is None:
	...
	expected_panels = EXPECTED_CATALOG[kind].get(key)
...
def validate_catalog(sheets: tuple[VisualSheet, ...]) -> None:
	"""Require one and only one complete sheet for every runtime asset."""
...
def render_typescript(sheets: tuple[VisualSheet, ...]) -> str:
	...
	lines = [
		...
		'export type WorldArtworkId = "tissue_cell" | "tumor_source" | "tumor_cluster" | "blood_exit";',
		"",
...
def main() -> None:
	...
	OUTPUT_FILE.write_text(text, encoding="ascii")
